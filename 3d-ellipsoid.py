...
# --- 1. Define Ellipsoid and Helper Functions ---

# Define the parameters for an ellipsoid
a, b, c = 3.0, 2.0, 1.5

def get_ellipsoid_points(resolution=200):
    """Generates a mesh of points on the ellipsoid surface."""
...
def get_surface_normal(x, y, z):
    """Calculates the normal vector at a point on the ellipsoid."""
    normal = np.array([2 * x / a**2, 2 * y / b**2, 2 * z / c**2])
    norm_mag = np.linalg.norm(normal)
    return normal / norm_mag if norm_mag > 0 else normal
...
# Plot the original ellipsoid with transparency
x_orig, y_orig, z_orig = get_ellipsoid_points()
ax.plot_surface(x_orig, y_orig, z_orig, color='blue', alpha=0.1, rstride=5, cstride=5)
...
def update(frame):
    ...
    # --- b. Find the rim on the ellipsoid ---
    ellipsoid_points_flat = np.vstack([x_orig.flatten(), y_orig.flatten(), z_orig.flatten()]).T
    surface_normals = np.apply_along_axis(lambda p: get_surface_normal(p[0], p[1], p[2]), 1, ellipsoid_points_flat)
    dot_products = np.dot(surface_normals, proj_normal)
    rim_indices = np.where(np.abs(dot_products) < 0.01)[0]
    rim_points = ellipsoid_points_flat[rim_indices]
    ...
    # The plane is shifted along the normal for better visibility
    plane_dist = -4.0
    outline_points = []
    for p in rim_points:
        # Project onto a plane at distance 'plane_dist' from origin
    ...
    rim_plot.set_data(rim_points_sorted[:, 0], rim_points_sorted[:, 1])
    rim_plot.set_3d_properties(rim_points_sorted[:, 2])
...
ani.save("ellipsoid_shadow_reconstruction.gif", writer=writer, dpi=60)